...
function makeWord(lettersObject) {
  ...
  const maxPosition = Math.max(
    ...letters.flatMap((letter) => lettersObject[letter])
  );

  let word = '';

  const appendLetterIfPositionMatches = (letter, index) => {
  ...
  for (let i = 0; i <= maxPosition; i += 1) {
    letters.forEach((letter) => appendLetterIfPositionMatches(letter, i));
  }
